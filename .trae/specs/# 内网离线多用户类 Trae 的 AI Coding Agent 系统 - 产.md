...
## Background & Context
- 离线 RAG 经验：embedding 层须"多后端 + 自动降级 + 显式传入向量"，避免隐式联网；无 LLM 路径仍须输出可读中间产物。
- LangGraph 经验：跨节点读取/条件边/鉴权字段必须显式进 State schema。
- Windows/PowerShell 经验：shell 命令须按目标 shell 语法生成，避免 cmd/Bash 混用。
- 部署目标：多卡内网 GPU 服务器（单卡显存几十 GB 量级），员工机浏览器访问。
