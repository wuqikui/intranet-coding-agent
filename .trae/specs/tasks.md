...
- **Completion Evidence**:
  - 目录结构 frontend/backend/inference/knowledge/tools/deploy/docs/.trae/specs 已创建
  - .env.example 含全部配置项（端口/模型/显存/知识库/沙箱/鉴权/审计/出网维护）
  - docker-compose.yml 含 frontend/backend/inference/sandbox 四服务 + volumes + networks
  - Makefile 含 up/down/build/rebuild/logs/init/clean/test/dev-frontend/dev-backend 目标
  - .gitignore 覆盖 Python/Node/.env/data/IDE/OS
